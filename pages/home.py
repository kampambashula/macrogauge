...
def run():
    ...
    for key in DATA:
        ...
        latest_dates.append(pd.to_datetime(DATA[key][col], errors='coerce').max())
    ...
    col1.button("📄 Monthly Macro Snapshot", use_container_width=True)
    col2.button("🧠 Macro Risk Monitor", use_container_width=True)
    col3.button("🏛️ Fiscal Dashboard", use_container_width=True)

    col4, col5 = st.columns(2)
    col4.button("📈 Commodities", use_container_width=True)
    col5.button("📊 Data Explorer (Coming Soon)", use_container_width=True, disabled=True)

    st.caption(
        "MacroGauge is a macro intelligence system and soverign risk terminal"
    )
